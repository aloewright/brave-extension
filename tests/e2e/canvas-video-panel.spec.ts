...
type PanelMessage = { type?: string; captureID?: string | null; videoID?: string };
...
test("Canvas video panel keeps downloads outside Keepout and tracks each video independently", async ({
  ...
  await expect(panel.getByText("Downloads folder · outside the encrypted vault", { exact: false })).toBeVisible();
  ...
  await panel.getByLabel("Note title").fill("Saved while video downloads");
