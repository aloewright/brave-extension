...
describe("SettingsPanel — active backend ring color (PDX-124)", () => {
  it("uses --tw-ring-color (not the invalid `ringColor` key)", () => {
    const style = activeBackendStyle("claude", "claude") as Record<
      string,
      unknown
    >;
  ...
  it("maps the ring color to the backend's brand color", () => {
    for (const backend of [
      ...
      const style = activeBackendStyle(backend, backend) as Record<
        string,
        unknown
      >;
...
describe("SettingsPanel — async action feedback", () => {
  it("renders the settings groups as accordions", async () => {
    const { host, cleanup } = await renderSettingsPanel();
    try {
      const summaries = Array.from(host.querySelectorAll("summary")).map(
        (node) => node.textContent?.replace(/\s+/g, " ").trim(),
      );

      expect(host.querySelectorAll("details").length).toBeGreaterThanOrEqual(
        12,
      );
      expect(summaries.some((text) => text?.includes("Appearance"))).toBe(true);
      expect(summaries.some((text) => text?.includes("Paths"))).toBe(true);
      expect(summaries.some((text) => text?.includes("MCP Servers"))).toBe(
        true,
      );
      expect(summaries.some((text) => text?.includes("Sidebar UX"))).toBe(true);
      expect(
  ...
  it("renders a loading glyph and disables Doppler OAuth login", async () => {
    ...
    try {
      ...
      const status = host.querySelector(
        '[aria-label="Connecting Doppler OAuth"]',
      );
