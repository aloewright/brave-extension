...
This uses the Canvas session already open in the browser; no Canvas API key or AI provider is involved. For Canvas File image URLs, the signed-in tab requests the read-only `/api/v1/files/:id/public_url` endpoint, and the extension downloads that signed image with cookies omitted. The signed URL is transient: it is not logged, displayed, or included in the saved note. Other same-origin images are read in the signed-in tab. Image bytes go only to Keepout's authenticated loopback API, never a cloud conversion service. Update Keepout to a build advertising `pageCaptureVersion: 1` at `/v1/captures/status`; the Local API must be enabled for **This Mac only** and the vault unlocked.

The panel also lists detected **Videos**, each with a **Download video** button. Canvas files are resolved through the signed-in tab and downloaded by the browser to **Downloads/Canvas**; Vimeo embeds use the existing local `yt-dlp`/FFmpeg helper and save to **Downloads**. Downloads are separate from the note save and are **outside the encrypted vault**. The panel shows started, completed, and retryable failures separately; closing it does not cancel an already-started download. Native Vimeo downloads need the installed helper plus `yt-dlp`/FFmpeg. No browser cookies are exported to the helper: only the embed URL and Canvas origin are provided. A video that requires separate Vimeo authentication, DRM, or an unavailable download stays unavailable; direct HLS/DASH playlists are not treated as downloadable video files. The extension does not bypass access restrictions.

Limits: one rendered page, 256 KB of Markdown, 32 raster images, 4 MB per image and 8 MB total, plus up to 16 video candidates. PNG, JPEG, GIF, WebP, HEIC and HEIF are accepted; the saved note retains video/source-page links and tables are flattened into readable rows. Video bytes never enter the page JSON, so the image-size limits do not restrict lecture downloads. A missing, blocked, or unsupported image stops the note import with an error instead of silently saving an incomplete page. Retry reuses the same capture ID and does not duplicate a successful note save. Course-wide crawling and interactive Canvas activities are not imported. The extension holds video URLs transiently in memory; the panel receives only video IDs/titles, never those URLs. **Browser Downloads can retain the original/signed URL in its normal local download history**, and the downloaded file is unencrypted. These downloads are not covered by Keepout vault encryption.
