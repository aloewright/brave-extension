...
Limits: one rendered page, 256 KB of Markdown, 32 raster images, 4 MB per image and 8 MB total, plus up to 16 video candidates. PNG, JPEG, GIF, WebP, HEIC and HEIF are accepted; the saved note retains video links and tables are flattened into readable rows. Video bytes never enter the page JSON, so the image-size limits do not restrict lecture downloads. A missing, blocked, or unsupported image stops the note import with an error instead of silently saving an incomplete page. Retry reuses the same capture ID and does not duplicate a successful note save. Course-wide crawling and interactive Canvas activities are not imported. Signed Canvas video URLs are held only during the download request; the panel receives only video IDs/titles, never those URLs. Browser Downloads keeps its normal local download history.
