...
function writeHostWrapper() {
  // Keep the executable launcher outside the source checkout. macOS can deny
  // GUI applications permission to execute helpers from Documents (and moved
  // checkouts leave a stale absolute manifest path behind). The app-support
  // directory is stable across repo moves and is already used for host state.
  const wrapperDir = join(homedir(), ".ai-dev-sidebar")
  const wrapperPath = join(wrapperDir, "ai-dev-host.sh")
