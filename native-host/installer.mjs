...
export function nativeHostLauncherPath(home = homedir()) {
  return join(configDir(home), "native-host")
}
...
/**
 * Build the native-messaging launcher installed outside the repository.
 *
 * Browser processes do not inherit the user's interactive shell PATH, so the
 * launcher records the Node binary used during installation. Keeping this
 * machine-specific file under ~/.config prevents install-host from modifying
 * the tracked checkout every time the repo or Node runtime moves.
 */
export function buildNativeHostLauncherScript(nodeBin, hostScript) {
  const hostDir = dirname(hostScript)
  return `#!/bin/bash
# Auto-generated by scripts/install-native-host.mjs.
# Re-running install regenerates this with the current Node and checkout paths.
export PATH="/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:$PATH"
HOST_DIR=${shellQuote(hostDir)}
...
export function writeNativeHostLauncher({ nodeBin, hostScript, home = homedir() }) {
  ensureDir(configDir(home))
  const path = nativeHostLauncherPath(home)
  const next = buildNativeHostLauncherScript(nodeBin, hostScript)
  if (existsSync(path)) {
    try {
      const cur = readFileSync(path, "utf-8")
      if (cur === next) {
...
export function removeNativeHostLauncher(home = homedir()) {
  const path = nativeHostLauncherPath(home)
  if (existsSync(path)) {
    try {
      unlinkSync(path)
      ...
      return { path, changed: false, error: err instanceof Error ? err.message : String(err) }
    }
  }
  return { path, changed: false }
}
