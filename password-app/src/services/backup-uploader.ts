...
export type RemoteBackupFetch = (input: RequestInfo | URL, init?: RequestInit) => Promise<Response>;

export interface RemoteBackupTransferOptions {
  fetcher?: RemoteBackupFetch;
}

const defaultRemoteFetch: RemoteBackupFetch = (input, init) => fetch(input, init);
...
async function ensureWebDavDirectory(
  ...
  authHeader: string,
  fetcher: RemoteBackupFetch = defaultRemoteFetch
): Promise<void> {
  ...
  for (const segment of segments) {
    ...
    throw new Error(`WebDAV directory creation failed for ${current}: ${response.status}${message ? ` ${message}` : ''}`);
  }
}
...
async function putToWebDav(
  ...
  if (remoteDir) {
    if (ensuredDirectories) {
      await ensureWebDavDirectoryCached(config.baseUrl, remoteDir, authHeader, ensuredDirectories, fetcher);
    } else {
      await ensureWebDavDirectory(config.baseUrl, remoteDir, authHeader, fetcher);
    }
  }

  const response = await fetcher(buildWebDavUrl(config.baseUrl, remoteFilePath), {
    method: 'PUT',
    headers: {
      Authorization: authHeader,
...
async function uploadToWebDav(
  ...
  await putToWebDav(config, fileName, archive, { contentType: 'application/zip' }, undefined, fetcher);
  return {
    provider: 'webdav',
...
async function listWebDavEntries(
  ...
  const authHeader = toBasicAuthHeader(config.username, config.password);
  const response = await fetcher(buildWebDavUrl(config.baseUrl, targetFullPath), {
    method: 'PROPFIND',
    headers: {
      Authorization: authHeader,
      Depth: '1',
...
async function downloadFromWebDav(
  ...
  const response = await fetcher(buildWebDavUrl(config.baseUrl, remotePath), {
    method: 'GET',
    headers: {
      Authorization: authHeader,
    },
...
function resolveConfiguredDestinationAdapter(
  ...
  if (destination.type === 'webdav') {
    const fetcher = options.fetcher || defaultRemoteFetch;
    return {
      provider: 'webdav',
      config: destination.destination as WebDavBackupDestination,
      upload: (config, archive, fileName) => uploadToWebDav(config as WebDavBackupDestination, archive, fileName, fetcher),
      putFile: (config, relativePath, bytes, options) => putToWebDav(config as WebDavBackupDestination, relativePath, bytes, options, undefined, fetcher),
      list: (config, relativePath) => listWebDavEntries(config as WebDavBackupDestination, relativePath, fetcher),
      download: (config, relativePath) => downloadFromWebDav(config as WebDavBackupDestination, relativePath, fetcher),
      deleteFile: (config, relativePath) => deleteFromWebDav(config as WebDavBackupDestination, relativePath, fetcher),
      exists: (config, relativePath) => existsInWebDav(config as WebDavBackupDestination, relativePath, fetcher),
    };
  }
  if (destination.type === 's3') {
    return {
...
export function createRemoteBackupTransferSession(
  ...
  const adapter = resolveConfiguredDestinationAdapter(destination, options);
  const ensuredDirectories = adapter.provider === 'webdav' ? new Set<string>() : null;
  const webDavFetcher = options.fetcher || defaultRemoteFetch;
  ...
  const putFile = async (relativePath: string, bytes: Uint8Array, options: RemoteBackupFilePutOptions = {}): Promise<void> => {
    ...
    if (adapter.provider === 'webdav' && ensuredDirectories) {
      await putToWebDav(adapter.config as WebDavBackupDestination, normalized, bytes, options, ensuredDirectories, webDavFetcher);
      return;
    }
...
export async function uploadBackupArchive(
  ...
  return createRemoteBackupTransferSession(destination, options).uploadArchive(archive, fileName);
}
...
export async function listRemoteBackupEntries(
  ...
  return createRemoteBackupTransferSession(destination, options).list(relativePath);
}
...
export async function downloadRemoteBackupFile(
  ...
  return createRemoteBackupTransferSession(destination, options).download(relativePath);
}
...
export async function deleteRemoteBackupFile(
  ...
  await createRemoteBackupTransferSession(destination, options).deleteFile(normalized);
}
...
export async function remoteBackupFileExists(
  ...
  const normalized = normalizeRelativePath(relativePath);
  return createRemoteBackupTransferSession(destination, options).exists(normalized);
}
...
export async function uploadRemoteBackupFile(
  ...
  bytes: Uint8Array,
  options: RemoteBackupFilePutOptions = {},
  transferOptions: RemoteBackupTransferOptions = {}
): Promise<void> {
  const normalized = normalizeRelativePath(relativePath);
  await createRemoteBackupTransferSession(destination, transferOptions).putFile(normalized, bytes, options);
}
