...
export class GoBackupTransferRunner {
  ...
  private async restoreRemoteBackup(request: Request): Promise<Response> {
    ...
    try {
      ...
      const remoteFile = await downloadRemoteBackupFile(destination, path, getBackupTransferOptions(this.env, destination));
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === '/internal/download-remote-attachment') {
      ...
      const file = await downloadRemoteBackupFile(
        ...
        getBackupTransferOptions(this.env, body.destination)
      ).catch(() => null);
      if (!file) {
    ...
    const remoteSession = createRemoteBackupTransferSession(
      body.destination,
      getBackupTransferOptions(this.env, body.destination)
    );
    let uploaded = 0;
