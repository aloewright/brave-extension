...
import {
  ...
  handleBootstrapAdminCredentialReset,
...
export async function handlePublicRoute(
  ...
  if (path === '/api/admin/backup/bootstrap' && method === 'POST') {
    const blocked = await enforcePublicRateLimit('public-sensitive', LIMITS.rateLimit.sensitivePublicRequestsPerMinute);
    if (blocked) return blocked;
    return handleBootstrapBackupSettings(request, env);
  }

  if (path === '/api/admin/bootstrap/reset-admin' && method === 'POST') {
    const blocked = await enforcePublicRateLimit('public-sensitive', LIMITS.rateLimit.sensitivePublicRequestsPerMinute);
    if (blocked) return blocked;
    return handleBootstrapAdminCredentialReset(request, env);
  }
