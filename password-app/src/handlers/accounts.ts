...
export async function handleRegister(request: Request, env: Env): Promise<Response> {
  ...
  if (userCount === 0) {
    const bootstrapAdminEmail = String(env.BOOTSTRAP_ADMIN_EMAIL || '').trim().toLowerCase();
    if (bootstrapAdminEmail && email !== bootstrapAdminEmail) {
      return errorResponse('First admin registration is restricted to the configured bootstrap admin email', 403);
    }

    const bootstrapInviteCode = String(env.BOOTSTRAP_INVITE_CODE || '').trim();
    if (bootstrapInviteCode && inviteCode !== bootstrapInviteCode) {
      return errorResponse('Valid invite code is required for first admin registration', 403);
    }
    user.role = 'admin';
...
async function getVaultResetImpact(storage: StorageService, userId: string): Promise<{
  ciphers: number;
  folders: number;
  sends: number;
  attachments: number;
  accountPasskeys: number;
}> {
  const [ciphers, folders, sends, attachmentsByCipher, accountPasskeys] = await Promise.all([
    storage.getAllCiphers(userId),
    storage.getAllFolders(userId),
    storage.getAllSends(userId),
    storage.getAttachmentsByUserId(userId),
    storage.countAccountPasskeyCredentialsByUserId(userId).catch(() => 0),
  ]);
  let attachments = 0;
  for (const rows of attachmentsByCipher.values()) {
...
function hasVaultResetImpact(impact: Awaited<ReturnType<typeof getVaultResetImpact>>): boolean {
  return impact.ciphers > 0 || impact.folders > 0 || impact.sends > 0 || impact.attachments > 0 || impact.accountPasskeys > 0;
}

function readBootstrapToken(request: Request, headerName: string): string {
  const authHeader = String(request.headers.get('Authorization') || '').trim();
  const bearerToken = authHeader.toLowerCase().startsWith('bearer ')
    ? authHeader.slice(7).trim()
    : '';
  return bearerToken || String(request.headers.get(headerName) || '').trim();
}

export async function handleBootstrapAdminCredentialReset(request: Request, env: Env): Promise<Response> {
  const expectedToken = String(env.ADMIN_RESET_BOOTSTRAP_TOKEN || '').trim();
  if (!expectedToken) return errorResponse('Not found', 404);
  if (readBootstrapToken(request, 'X-Admin-Reset-Bootstrap-Token') !== expectedToken) {
  ...
  const configuredAdminEmail = String(env.BOOTSTRAP_ADMIN_EMAIL || '').trim().toLowerCase();
  const email = String(body.email || configuredAdminEmail || '').trim().toLowerCase();
  if (!email || (configuredAdminEmail && email !== configuredAdminEmail)) {
    return errorResponse('Admin reset is restricted to the configured bootstrap admin email', 403);
  }
  if (!body.masterPasswordHash || !body.key || !body.keys?.encryptedPrivateKey || !body.keys?.publicKey) {
    return errorResponse('Email, masterPasswordHash, key, and keys are required', 400);
  }
  if (!looksLikeEncString(body.key)) {
    return errorResponse('key is not a valid encrypted string', 400);
  }
  if (!looksLikeEncString(body.keys.encryptedPrivateKey)) {
    return errorResponse('encryptedPrivateKey is not a valid encrypted string', 400);
  }
  const kdfErr = validateKdfParams(body.kdf, body.kdfIterations, body.kdfMemory, body.kdfParallelism);
  if (kdfErr) return errorResponse(kdfErr, 400);
  ...
  if (user.role !== 'admin') return errorResponse('Bootstrap reset target must be an admin', 403);

  const impact = await getVaultResetImpact(storage, user.id);
  if (hasVaultResetImpact(impact) && !body.allowVaultKeyReset) {
    return jsonResponse({
      error: 'Vault key reset confirmation is required',
      impact,
    }, 409);
  }

  const passkeys = await storage.getAccountPasskeyCredentialsByUserId(user.id).catch(() => []);
  for (const passkey of passkeys) {
    await storage.deleteAccountPasskeyCredential(user.id, passkey.id).catch(() => false);
  }

  const now = new Date().toISOString();
  user.name = String(body.name || user.name || email).trim();
  user.masterPasswordHint = normalizeMasterPasswordHint(body.masterPasswordHint);
  user.masterPasswordHash = await auth.hashPasswordServer(body.masterPasswordHash, user.email);
  user.key = body.key;
  user.privateKey = body.keys.encryptedPrivateKey;
  user.publicKey = body.keys.publicKey;
  user.kdfType = body.kdf ?? 0;
  user.kdfIterations = body.kdfIterations ?? LIMITS.auth.defaultKdfIterations;
  user.kdfMemory = body.kdfMemory;
  user.kdfParallelism = body.kdfParallelism;
  user.securityStamp = generateUUID();
  user.status = 'active';
  ...
  await safeWriteAuditEvent(env, {
    actorUserId: user.id,
    action: 'user.password.bootstrap_reset',
    targetType: 'user',
    targetId: user.id,
    category: 'security',
    level: 'security',
    metadata: {
      email: user.email,
      impact,
      removedAccountPasskeys: passkeys.length,
      ...auditRequestMetadata(request),
    },
  });

  return jsonResponse({
    object: 'admin-credential-reset',
    ok: true,
    email: user.email,
    impact,
    removedAccountPasskeys: passkeys.length,
