...
import {
  type RemoteBackupTransferOptions,
...
const INTERNAL_WEBDAV_SERVICE_HOSTS = new Set(['dav.lazee.workers.dev']);

export function getBackupTransferOptions(
  env: Env,
  destination: BackupDestinationRecord
): RemoteBackupTransferOptions | undefined {
  if (destination.type !== 'webdav' || !env.DAV_WEBDAV) return undefined;
  const config = destination.destination as WebDavBackupDestination;
  try {
    const host = new URL(String(config.baseUrl || '')).hostname.toLowerCase();
    if (!INTERNAL_WEBDAV_SERVICE_HOSTS.has(host)) return undefined;
  } catch {
    return undefined;
  }
  return {
    fetcher: (input, init) => env.DAV_WEBDAV!.fetch(input, init),
  };
}
...
export async function executeConfiguredBackup(
  ...
  try {
    ...
    const transferOptions = getBackupTransferOptions(env, destination);
    const remoteSession = createRemoteBackupTransferSession(destination, transferOptions);
    ...
    try {
      ...
      prunedFileCount = await pruneRemoteBackupArchives(destination, destination.schedule.retentionCount, archive.fileName, transferOptions);
    } catch (error) {
...
export async function handleBootstrapBackupSettings(request: Request, env: Env): Promise<Response> {
  const expectedToken = String(env.BACKUP_BOOTSTRAP_TOKEN || '').trim();
  ...
  const bearerToken = authHeader.toLowerCase().startsWith('bearer ')
    ...
    : '';
  const providedToken = bearerToken || String(request.headers.get('X-Backup-Bootstrap-Token') || '').trim();
  if (providedToken !== expectedToken) return errorResponse('Forbidden', 403);

  let body: BackupSettingsInput & { runNow?: boolean; destinationId?: string | null };
  try {
    body = await request.json<BackupSettingsInput & { runNow?: boolean; destinationId?: string | null }>();
  } catch {
    return errorResponse('Backup settings payload is invalid', 400);
  }
  ...
  await saveBackupSettings(storage, env, next);
  await writeAuditLog(storage, null, 'admin.backup.settings.bootstrap', 'backup', null, {
    destinationCount: next.destinations.length,
    scheduledDestinationCount: next.destinations.filter((destination) => destination.schedule.enabled).length,
  }, request);

  if (!body.runNow) {
    return jsonResponse({ object: 'backup-bootstrap', settings: next }, 200);
  }

  try {
    const destinationId = body.destinationId || next.destinations[0]?.id || null;
    const result = await executeConfiguredBackup(env, storage, null, 'manual', destinationId, null, null, {
      bootstrap: true,
    });
    const updatedSettings = await loadBackupSettings(storage, env, 'UTC');
    return jsonResponse({ object: 'backup-bootstrap', result, settings: updatedSettings }, 200);
  } catch (error) {
    return errorResponse(error instanceof Error ? error.message : 'Backup upload failed', 500);
  }
}
...
export async function handleListAdminRemoteBackups(request: Request, env: Env, actorUser: User): Promise<Response> {
  ...
  try {
    ...
    const listing = await listRemoteBackupEntries(destination, url.searchParams.get('path') || '', getBackupTransferOptions(env, destination));
    return jsonResponse({
...
export async function handleDownloadAdminRemoteBackup(request: Request, env: Env, actorUser: User): Promise<Response> {
  ...
  try {
    ...
    const destination = requireBackupDestination(settings, url.searchParams.get('destinationId') || null);
    const remoteFile = await downloadRemoteBackupFile(destination, path, getBackupTransferOptions(env, destination));
...
export async function handleDeleteAdminRemoteBackup(request: Request, env: Env, actorUser: User): Promise<Response> {
  ...
  try {
    ...
    const path = ensureRemoteRestoreCandidate(url.searchParams.get('path') || '');
    const destination = requireBackupDestination(settings, url.searchParams.get('destinationId') || null);
    await deleteRemoteBackupFile(destination, path, getBackupTransferOptions(env, destination));
