...
import { sendMediaHostMessage, startMediaDownload } from "./lib/media-download";
import {
  saveKeepoutCapture,
...
async function importCanvasVideo(
  ...
  job.promise = (async () => {
    ...
    if (video.kind === "vimeo") {
      const native = await sendMediaHostMessage({
...
chrome.commands.onCommand.addListener(async (command) => {
  if (command === "toggle-sidebar") {
    ...
    if (tab?.url && tab?.title) {
      ...
      await saveLinkToLibrary(tab.url, tab.title, tags);
      // Best-effort visual confirmation on the toolbar icon.
      try {
        await chrome.action.setBadgeBackgroundColor({ color: "#2c50cd" });
        await chrome.action.setBadgeText({ text: tags.includes("pdf") ? "PDF" : "✓" });
        setTimeout(() => {
          void chrome.action.setBadgeText({ text: "" });
        }, 2000);
      } catch {
