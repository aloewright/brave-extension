...
/**
 * CloudOS sync — pushes conversations to the cloudos-notes worker so they're
 * persisted in D1 and auto-embedded into the cloudos-embeddings Vectorize
 * index. The same vector store powers cloudos search, the orchestrator agent,
 * and anything else in the cloudos ecosystem — giving you a "consistent
 * embedded memory running through everything."
 *
 * Strategy:
 *   - One note per "session" per backend. A session begins when there's no
 *     current note for that backend, and ends at a clear marker.
 *   - First message in a session: POST /api/notes → store the new note id
 *     keyed by `${backend}:${sessionStart}` in chrome.storage.local
 *   - Subsequent messages: PUT /api/notes/:id (debounced 3s) with the full
 *     serialized session
 *   - Clear marker → forget the current note id so the next message starts
 *     a new session
 *   - On successful PUT, optionally prune older messages from local storage
 *     to keep browser space low (only messages OLDER than the most recent
 *     clear marker for that backend are eligible — never prune live history)
 */
