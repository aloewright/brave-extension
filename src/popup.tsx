...
function readThirdPartyCookiePrompt(): ThirdPartyCookiePrompt | null {
  ...
  return {
    ...
    embeddedName:
      params.get("embeddedName") || companyNameForDomain(embeddedDomain),
  };
}
...
function idleRecordingState(lastError: string): RecordingState {
  return {
    active: false,
    paused: false,
    source: null,
...
function Popup() {
  ...
  const setStartError = (lastError: string) => {
    setState((s) => (s ? { ...s, lastError } : idleRecordingState(lastError)));
  };
  ...
  const startRecording = async () => {
    try {
      ...
      setStartError((err as Error).message || "Start failed");
    }
  };
    ...
    return (
      <div className="w-[340px] bg-bg text-fg p-5">
        ...
        <h1 className="text-lg font-semibold leading-tight">
          Allow {thirdPartyCookiePrompt.embeddedName} cookies while using{" "}
          {thirdPartyCookiePrompt.siteName}?
        </h1>
        <p className="text-sm text-fg/60 mt-3 leading-relaxed">
          Brave Dev Extension blocks embedded third-party cookie access by
          default. Allowing this lets {thirdPartyCookiePrompt.embeddedName}
          receive and set its cookies when it is embedded on{" "}
          {thirdPartyCookiePrompt.siteName}.
        </p>
        <p className="text-xs text-fg/45 mt-3 leading-relaxed">
          Only grant this when the embedded company is needed for sign-in,
          checkout, chat, or another feature you expect. Marketing and analytics
          cookies are usually better left blocked.
        </p>
