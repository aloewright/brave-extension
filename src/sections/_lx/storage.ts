...
export async function getAll(): Promise<StorageSchema> {
  const profiles = (await storage.get<Profile[]>(KEYS.profiles)) ?? DEFAULT_STORAGE.profiles
  const groups = (await storage.get<Group[]>(KEYS.groups)) ?? DEFAULT_STORAGE.groups
  const collectedLinks = (await storage.get<CollectedLink[]>(KEYS.collectedLinks)) ?? DEFAULT_STORAGE.collectedLinks
  const settings = normalizeSettings(await storage.get<Settings>(KEYS.settings))
  const extensionLastUsed = (await storage.get<Record<string, string>>(KEYS.extensionLastUsed)) ?? DEFAULT_STORAGE.extensionLastUsed
...
export async function getLastUsed(): Promise<Record<string, string>> {
  return (await storage.get<Record<string, string>>(KEYS.extensionLastUsed)) ?? {}
}
