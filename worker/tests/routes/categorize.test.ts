...
describe("POST /api/bookmarks/categorize", () => {
  ...
  it("rejects items without the required id/title/url shape", async () => {
    ...
    const res = await app.fetch(
      jsonReq("http://x/api/bookmarks/categorize", { items: [{ id: "a", title: "ok" }] }),
      env
    )
    expect(res.status).toBe(400)
  })
  ...
  it("rejects batches above MAX_BATCH with too_many_items", async () => {
    ...
    const res = await app.fetch(
      jsonReq("http://x/api/bookmarks/categorize", { items }),
      env
    )
  ...
  it("invokes env.AI.run with @cf/openai/gpt-oss-120b through gateway x", async () => {
    ...
    expect(body.proposals[0]).toMatchObject({ id: "bm1", category: "Tech News", confidence: "high" })
  ...
  it("returns 502 on AI gateway failure", async () => {
    const env = makeEnv()
    env.AI = { run: vi.fn().mockRejectedValue(new Error("rate limited")) } as unknown as Ai
    const res = await app.fetch(
  ...
  it("falls back to 'Uncategorized' when the model returns garbage", async () => {
    ...
    expect(res.status).toBe(200)
    const body = (await res.json()) as any
    expect(body.proposals[0]).toEqual({ id: "b1", category: "Uncategorized", confidence: "low" })
  })
